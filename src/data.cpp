...
void initialize_data_A() {
    try {
        if (A == nullptr)
            A = new std::vector<int>[sim.cfg.I]();
        if (Ac == nullptr)
            Ac = new std::vector<int>[sim.cfg.J]();
        if (SA == nullptr)
            SA = new std::vector<int>[sim.cfg.I]();
        if (SAc == nullptr)
            SAc = new std::vector<int>[sim.cfg.J]();
        if (offsetarrayA == nullptr)
            offsetarrayA = new int[sim.cfg.I]();
        if (offsetarrayAc == nullptr)
            offsetarrayAc = new int[sim.cfg.J]();
        if (SAindex == nullptr)
            SAindex = new int[sim.cfg.I]();
...
void initialize_data_B() {
    try {
        if (B == nullptr)
            B = new std::vector<int>[sim.cfg.I]();
        if (Bc == nullptr)
            Bc = new std::vector<int>[sim.cfg.J]();
        if (SB == nullptr)
            SB = new std::vector<int>[sim.cfg.I]();
        if (SBc == nullptr)
            SBc = new std::vector<int>[sim.cfg.J]();
        if (offsetarrayB == nullptr)
            offsetarrayB = new int[sim.cfg.J]();
        if (offsetarrayBc == nullptr)
            offsetarrayBc = new int[sim.cfg.K]();
        if (SBcindex == nullptr)
            SBcindex = new int[sim.cfg.K]();
    } catch (const std::bad_alloc &e) {
        std::cerr << "Error allocating memory for " << e.what() << std::endl;
        std::exit(1);
    }
